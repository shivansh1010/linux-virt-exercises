	...
	for (;;) {
		...
		case KVM_EXIT_IO:
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
				...
				char *p = (char *)vcpu->kvm_run;
				uint32_t a = *(uint32_t*)(p + vcpu->kvm_run->io.data_offset);
				printf("%u\n", a);
				fflush(stdout);
				continue;
			}
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN
				...
				char *p = (char *)vcpu->kvm_run;
				*(p + vcpu->kvm_run->io.data_offset) = num_vm_exits;
				continue;
			}
			// timestamp
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN
				...
				char *p = (char *)vcpu->kvm_run;
				struct kvm_clock_data ts;
				if (ioctl(vm->fd, KVM_GET_CLOCK, &ts) < 0) {
					perror("KVM_GET_CLOCK");
					exit(1);
				}
				*(uint32_t*)(p + vcpu->kvm_run->io.data_offset) = (uint32_t)ts.clock;
				continue;
			}
			// gva to hva outb
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
				...
				char *p = (char *)vcpu->kvm_run;
				uint32_t *gva = (uint32_t *)(p + vcpu->kvm_run->io.data_offset);
				if (*gva > 2097152) {
					printf("Invalid GVA\n");
					addr = 0x0;
				} else {
					addr = *gva + *((uint32_t*)vm->mem);
				}
				continue;
			}
			// gva to hva inb
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN
			    && vcpu->kvm_run->io.port == 0xE5) {
				char *p = (char *)vcpu->kvm_run;
				*(uint32_t*)(p + vcpu->kvm_run->io.data_offset) = addr;
				continue;
			}
			// string
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
			    && vcpu->kvm_run->io.port == 0xE4) {
				char *pp = (char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset;
				printf("%s", vm->mem + *((uint32_t*)pp));
				fflush(stdout);
				continue;
			}
